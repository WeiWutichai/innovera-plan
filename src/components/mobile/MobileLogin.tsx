    ...
    <div style={{ position: "absolute", inset: 0, zIndex: 60, background: "var(--color-bg)", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ flex: 1, padding: "30px 26px", display: "flex", flexDirection: "column" }}>
        ...
        <form onSubmit={(e) => { e.preventDefault(); f.submit(); }}>
          ...
          <div className="field" style={{ marginBottom: f.error ? 12 : 24 }}>
            <label>{L.lg_pass}</label>
            <input className="input" type="password" value={f.password} onChange={(e) => f.setPassword(e.target.value)} />
          </div>
          {f.error && <div style={{ color: "var(--color-accent-700)", fontSize: 13, marginBottom: 16 }}>{f.error}</div>}
          <button type="submit" className="btn btn-primary" disabled={f.submitting} style={{ width: "100%", justifyContent: "center", height: 48 }}>{L.lg_signin}</button>
        </form>
        <div style={{ flex: 1 }} />
